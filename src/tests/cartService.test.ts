...
describe('CartService', () => {
  it('checkout simples', async () => {
    ...
    if (result.ok) {
      expect(result.value.totals.itemsTotal).toBe(200)
      expect(result.value.totals.grandTotal).toBe(200) // sem desconto/frete
    }
  })
  ...
  it('erro por carrinho vazio', async () => {
    ...
    if (!result.ok) expect(result.error).toBe('EMPTY_CART')
  })
})
